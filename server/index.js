...
app.post("/sett1value", function(req, res) {
    t1 = 0;
    let t1random = Math.floor(Math.random() * 100) + 1
...
app.post("/guessNumber", async function(req, res) {
    ...
    let t1random = rowsfrom.rows[0].number
    console.log(t1random)
    ...
    if (t1 < 10 && hintStr != "equals") {
        ...
        t1 = 0;
        newt1random = Math.floor(Math.random() * 100) + 1
        pgClient.query('DELETE FROM rgn WHERE rgnfor=$1', ["t1rn"])
...
app.get("/entered", function(req, res) {
    redisClient.hgetall('t1en', (err, values) => {
